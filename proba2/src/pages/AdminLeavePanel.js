...
const AdminLeavePanel = () => {
    ...
    useEffect(() => {
        const fetchLeaves = async () => {
            try {
                const response = await axios.get("http://localhost:8081/leave", { withCredentials: true });
                const leaveRequests = response.data.filter(leave => leave.status !== "rejected"); // Filtrujemy od razu
                setLeaves(leaveRequests);

                const userPromises = leaveRequests.map(async (leave) => {
                    if (!users[leave.user_id]) {
                        const userResponse = await axios.get(`http://localhost:8081/user/profile/${leave.user_id}`, { withCredentials: true });
    ...
    const updateLeaveStatus = async (id, status) => {
        try {
            await axios.put(`http://localhost:8081/leave/${id}/status`, { status }, { withCredentials: true });
