...
const KanbanBoard = () => {
  const { tasks, updateTask, showAllTasks, setShowAllTasks, loading, error } =
    useTasks(/*showAllTasks ? {} : { userId }*/);

  const stages = ["Planning", "In Progress", "Review", "Done"];
  const { currentUser } = useAuth(); // Get current user info
  const userId = currentUser?.id; // Pobranie userId (może być undefined, jeśli użytkownik nie jest zalogowany)
